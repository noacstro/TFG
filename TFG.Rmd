...
X_train = scaler.fit_transform(X_train)# Display predicted classes and associated probabilities for the first few samples
for i in range(5):  # Display probabilities for the first 5 samples
    print(f"Sample {i+1}: Predicted Class = {y_pred[i]}, Probabilities = {y_probabilities[i]}")
X_test = scaler.transform(X_test)

# Train SVM classifier with probability estimates enabled (Platt scaling)
svm_classifier = SVC(kernel='rbf', probability=True, random_state=42)
svm_classifier.fit(X_train, y_train)

# Make predictions and obtain probability estimates
y_pred = svm_classifier.predict(X_test)
y_probabilities = svm_classifier.predict_proba(X_test)
...
Primero eliminar variables (columnas) del final.

Las preguntas que no aplican a toda la muestra, eliminar salvo SALUD MENTAL (poner 0 en los NAs).

Eliminar las instancias de los 99.

Poner bien 0 y 1 de las variables que aparecen "mal" codificadas.

PCA y ciertas características de la muestra.


Hiperp: para C y gamma (desde 2^-5...2^5).
